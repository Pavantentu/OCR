...
class SHGFormDetector:
    ...
    def get_size_config(self, image_h, image_w):
        ...
        size_category = self.size_profile['category'] if self.size_profile else self._categorize_diagonal(img_diagonal)
    ...
    def detect_lines_and_intersections(self, image, img_name):
        ...
        # Dilate vertical lines slightly for connection detection
        v_dilated = cv2.dilate(filtered_v_mask, cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)), iterations=1)
        
        # Estimate minimum cell width from vertical lines
        min_cell_width = w // 10  # default fallback
    ...
    def trace_cells_from_intersections(self, h_lines, v_lines, intersections, img_name, image):
        ...
        def are_h_connected(a, b, tol=PIX_TOL):
            ...
            if abs(ha['position'] - hb['position']) > tol:
                log(f"      [H-CONN DEBUG] Y mismatch: ha_pos={ha['position']}, hb_pos={hb['position']}, diff={abs(ha['position'] - hb['position'])}")
                return False
            if not h_line_covers_point(ha, a['x'], tol) or not h_line_covers_point(hb, b['x'], tol):
                log(f"      [H-CONN DEBUG] Point not in line: ha covers a[{a['x']}]? {h_line_covers_point(ha, a['x'], tol)}, hb covers b[{b['x']}]? {h_line_covers_point(hb, b['x'], tol)}")
        ...
        def create_cell(tl, tr, bl, br, context):
            ...
            if not right_ok and top_ok and bottom_ok and left_ok:
                ...
                if x2 > img_w * 0.90:
                    ...
                    right_ok = True
                # Or check if there's a significant line detected (>=55% coverage minimum)
                elif right_cov >= 0.55:  # Require at least 55% for weak edge acceptance
                    log(f"  Right edge has strong partial coverage ({right_cov:.2f}) - accepting")
                    right_ok = True
                else:
                    log(f"  Right edge coverage too weak ({right_cov:.2f} < 0.55) - rejecting")
    ...
    def filter_and_extract_cells(self, image, cells, img_name):
        ...
        for cell in valid_cells:
            ...
            # ============================================
            padding_left = padding_left      # More padding on left
            padding_top = padding_top       # More padding on top
            padding_right = padding_right     # Normal padding on right
            padding_bottom = padding_bottom    # Normal padding on bottom

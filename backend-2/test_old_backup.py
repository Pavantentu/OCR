...
class SHGFormDetector:
    ...
    def remove_table_lines(self, image):
        """
        Remove table lines using strict mask mode - preserves exact shape and size.
        Uses the detected mask directly without rectangular dilation to maintain
        curved lines and exact shapes from real-life tables.
        """
        ...
        if self.debug:
            print("  Using strict mask mode - preserving exact line shapes (including curves)...")
        
        # === GET MASKS DIRECTLY ===
        # Use masks as-is to preserve exact shape and size
        # This ensures curved lines in real-life tables maintain their exact shape
        h_mask = self.horizontal_mask
        v_mask = self.vertical_mask
        
        # === CREATE PRECISE REMOVAL MASK ===
        # Use mask directly to preserve exact shape and size (including curved lines)
        # For strict mode: use the mask as-is without rectangular dilation
        # This preserves the exact shape of curved lines in real-life tables
        h_removal_mask = h_mask.copy()
        v_removal_mask = v_mask.copy()
        
        # Combine masks - preserves exact shape of both horizontal and vertical lines
        ...
        if self.debug:
            self.save_debug_image(final_removal_mask, f"line_removal_mask.jpg", 
                                f"Strict removal mask (exact shape preserved)")
        ...
        if self.debug:
            print(f"    ✓ Lines removed with strict mask (exact shape preserved)")
